...
const TIP_MESSAGE =
  "Вот мое расписание. Если ты хочешь создать подобное, используй @t_schedule_bot.";

export default async function inlineShare(ctx: InlineContext) {
  if (!ctx.inlineQuery.chat_type || ctx.inlineQuery.chat_type !== "private")
    return;
