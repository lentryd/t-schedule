...
function stringToHex(str: string): string {
    ...
    if (lightness.startsWith('darken-')) color.darken(parseInt(lightness.split('-')[1]));
    // Если надо, то осветляем цвет
    if (lightness.startsWith('brighten-')) color.brighten(parseInt(lightness.split('-')[1]));
 ...
 */
function cie94(color1: LCH, color2: LCH): number {
    // Вычисляем разницу по яркости (Lightness)
    let deltaL = color1.l - color2.l;

    // Вычисляем разницу по насыщенности (Chroma)
    let deltaC =
        Math.sqrt(color1.c * color1.c + color1.h * color1.h) - Math.sqrt(color2.c * color2.c + color2.h * color2.h);

    // Вычисляем разницу по оттенку (Hue)
    let deltaH = Math.sqrt(
        (color1.c - color2.c) * (color1.c - color2.c) + (color1.h - color2.h) * (color1.h - color2.h) - deltaC * deltaC
    );
 ...
 */
export default function nearestColor(color: string | null): number {
