...
import { LessonsTypesResponse, RaspListResponse, RaspResponse } from './wrapper';

interface StudentFormat {
    fullName: string;
    department: string;
}

/**
 * Форматирует информацию о студенте по его идентификатору
 */
export function formatStudent(id: number): StudentFormat | undefined;
/**
 * Форматирует информацию о студенте
 */
...
export function formatStudent(arg: number | Student): StudentFormat | undefined {
    ...
    return {
        ...
        department: student.course + ' курс ' + (student.spaceID === 1 ? '(Школа Икс)' : '(Т-университет)'),
    };
}
...
/**
 * Форматирует расписание для Google Calendar
 */
    ...
    return raspItem
        ...
        .map((item) => {
            ...
            if (item.info.type) {
                ...
                const lessonType = lessonsTypes?.find((type) => type.label === item.info.type);
                // Получаем аббревиатуру типа занятия
                let typeAbbr = lessonType?.abbreviation ?? item.info.type.match(REGEX_ABBR)?.[1] ?? item.info.type;
            ...
            if (item.info.teachers && item.info.teachers.length > 0) {
                const teacherLabel = item.info.teachers.length === 1 ? 'Преподаватель' : 'Преподаватели';
...
/**
 * Форматирует резервное расписание для Google Calendar
 */
export function formatRasp(rasp: RaspResponse['data']['rasp']): ScheduleFormat[] {
    return rasp.map((item) => {
        ...
        if (item.преподаватель) {
            const teacherLabel = item.преподаватель.split(',').length === 1 ? 'Преподаватель' : 'Преподаватели';
...
/**
 * Форматирует событие из Google Calendar в формат расписания
 */
export function formatEvent(event: calendar_v3.Schema$Event): ScheduleFormat & { id: string } {
    const startDateTime = event.start?.dateTime && new Date(event.start.dateTime).toISOString();
    const endDateTime = event.end?.dateTime && new Date(event.end.dateTime).toISOString();
