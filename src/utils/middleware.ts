...
interface Options<C> {
  ...
  docKey?:
    ...
    | ((ctx: C) => Promise<string | undefined>);
  defaultValue?:
    | object
    | ((ctx: C) => object | undefined)
    | ((ctx: C) => Promise<object | undefined>);
}
...
export default function firestoreMiddleware<C extends Context>(
  ...
  const defaultOpts = {
    name: "session",
    docKey: (ctx: C) =>
      (ctx.from && ctx.from.id.toString()) ||
      (ctx.inlineQuery?.from && ctx.inlineQuery.from.id.toString()),
  };
  ...
  return async (ctx, next) => {
    const options = {
      ...completeOpts,
      docKey: async (ctx: C) =>
        typeof completeOpts.docKey === "function"
          ? completeOpts.docKey(ctx)
          : completeOpts.docKey,
      defaultValue: async (ctx: C) =>
    ...
    Object.defineProperty(ctx, options.name, {
      get: () =>
        new Promise(async (resolve) => {
          if (data === undefined)
            data =
              (await collection.doc(key).get()).data() ??
              (await options.defaultValue(ctx));
          resolve(data);
        }),
